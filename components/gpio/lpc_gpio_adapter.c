...
#if (defined(FSL_FEATURE_SYSCON_HAS_PINT_SEL_REGISTER) && (FSL_FEATURE_SYSCON_HAS_PINT_SEL_REGISTER > 0U))
        SYSCON_AttachSignal(SYSCON, (pint_pin_int_t)gpioStateHandle->pin.pint.pintIndex,
                            (syscon_connection_t)HAL_INPUTMUX_GpioPortPinToPintsel(gpioStateHandle->pin.port,
